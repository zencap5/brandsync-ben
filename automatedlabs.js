  ...
  for (const entry of cursorTextArray) {
    const DUPLICATED_CURSOR_TEXT_STYLES = "opacity: 0; transition: opacity 0.25s; color: transparent; background-clip: text; -webkit-background-clip: text; background-position: center; background-size: cover; background-image: url(https://uploads-ssl.webflow.com/63ff44e96d759d26652026db/6400d8c47173b76f976646bb_BrandSync_Blend.png);";

    if (entry.strategy === "overlay") {
      const duplicatedCursorText = entry.baseElement.cloneNode(true);
      duplicatedCursorText.style = DUPLICATED_CURSOR_TEXT_STYLES;
      ...
      const innards = entry.baseElement.innerHTML
      entry.baseElement.innerHTML = ""
      span.innerHTML = innards
      entry.baseElement.appendChild(span)

      const duplicatedCursorText = document.createElement('span')
      duplicatedCursorText.innerHTML = innards
      entry.overlayElement = duplicatedCursorText;
      duplicatedCursorText.style = DUPLICATED_CURSOR_TEXT_STYLES + "grid-column: 1; grid-row: 1; filter: invert(1);";
      entry.baseElement.style = "display: grid;"
      entry.baseElement.appendChild(duplicatedCursorText)
    }
  }
  ...
  cursorTextArray
    .filter(entry => entry.strategy === "overlay")
    .forEach(entry => {
